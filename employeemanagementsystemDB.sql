...
--
-- This script contains SQL statements to create the tables, insert the provided data,
-- and then run the query to calculate the number of younger employees per department.
-- The statements are compatible with standard SQL, and can be executed in most
-- database environments like SQLite, MySQL, or PostgreSQL.
...
-- 1. Create the DEPARTMENT table and insert data
...
-- 2. Create the EMPLOYEE table and insert data
...
-- 3. Create the PAYMENTS table (although not used in the final query, it's good practice to include it based on the problem description)
...
-- 4. Final Query to solve the problem
SELECT
    e1.EMP_ID,
    e1.FIRST_NAME,
    e1.LAST_NAME,
